...
def stopDocker():
    ...
    logger.debug('StopDocker: docker address='+str(dockerExternalIP)+':'+str(ipDockerPortBind[serviceIP])+', service address='+str(serviceIP)+':'+str(servicePort))
...
def startDocker():
    ...
    try:
        ...
        logger.debug('StartDocker:'+ str(docker.errors.ContainerError))

    logger.debug('StartDocker: docker address='+str(dockerExternalIP)+':'+str(dockerPort)+', service address='+str(serviceIP)+':'+str(servicePort))
...
def requestDockerStart(ControllerIP,ControllerPort,RequesterIp,RequesterServPort,DockerImageName,ReqestedPiService):
    ...
    logger.debug('RequestStartDocker: '+str(RequesterIp)+' is requesting '+str(ControllerIP)+' to start a docker')
...
def requestDockerStop(ControllerIP,ControllerPort,RequesterIp):
    ...
    logger.debug('RequestStartDocker: '+str(RequesterIp)+' is requesting '+str(ControllerIP)+' to stop a docker')
        ...
        if status=='up' and host!=gateway and host!=ip:
            ...
            try:
                if urllib.request.urlopen("http://"+host+":"+str(servicePort)+"/battery").getcode()==200:
                    logger.debug('Neighbour:'+ str(host)+' is active')
                    neighbours.add(host)
            except:
                logger.debug('Neighbour:'+ str(host)+' is down')
        ...
        if offload == None or offload=='0': #offload is not set or set as False 
            if localDockerStatus==0: #local docker is not executing
                ...
                if responseData==True:
                    logger.debug('Controller: docker is running')
                    localDockerStatus=1
                else:
                    logger.debug('Controller: docker creation in failed')
            else: #local docker is executing
                logger.debug('Controller: docker is running locally')
            ...
            if localBatteryLevel != None: # Service is responding
                if int(localBatteryLevel) > 50: # Battery is high
                    if localDockerStatus==0 and remoteDockerStatus==0: # no local and remote docker has been running
                        ...
                        if responseData==True:
                            logger.debug('Controller: local docker is running')
                            localDockerStatus=1 #local docker is running
                        else:
                            logger.debug('Controller: local docker creation failed')
                    ...
                    if remoteDockerStatus==0: # No remote docker, want to create
                        ...
                        for PI in neighbourPIs:
                            ...
                            if remoteBatteryLevel != None and int(remoteBatteryLevel)>70: # trying to find a RPi with even higher battery level
                                ...
                                logger.debug('Controller: '+str(selectedRemotePi)+' is selected for docker exectuion')
                                break
                        if selectedRemotePi == None and localDockerStatus==0: # not found offloading destination and local docker is not running
                            ...
                            if responseData==True:
                                logger.debug('Controller: remote docker is running')
                                ...
                                logger.debug('Controller: terminated all locally run docker')
                            else:
                                logger.debug('Controller: remote docker creation failed')
                            time.sleep(30)     
                ...
                if remoteDockerStatus==1:
                    ...
                    if str(selectedRemotePi) in responseData2:
                        logger.debug('Controller: '+str(selectedRemotePi)+' is running the docker remotely')
                    else:
                        remoteDockerStatus=0
                    
            else: #service is not responding
               logger.debug('Controller: PiService is not responding')
               logger.debug('Controller: Program finishing...')
               break
