...
import json
#PiJuice
# mo from pijuice import PiJuice
#PiCamera
...
## mo    pijuice = PiJuice(1,0x14)
## mo    charge = pijuice.status.GetChargeLevel()
## mo    return str(charge['data'])
    charge = math.ceil(random.random() * 100)
    return str(charge)
...
def gpioAcFunc():
    ...
    ## mo GPIO.setup(int(content['repeller']), GPIO.OUT)
    ## mo time.sleep(15)
    ## mo GPIO.setup(int(content['repeller']), GPIO.IN)

    print ("mo: turn on repeller for 15 seconds")
    time.sleep(15)
    print ("mo: turn off repeller")
    return 'success'
...
def checkWithMotion(pir):
    global logger
    logger.info('Mot    ion: service started')
    print("Motion Called")
    try:
